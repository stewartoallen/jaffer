...
public class Rendezvous extends DNSConstants
{
     ...
     * @param type full qualified service name, such as <code>foobar._http._tcp.local.</code>.
	...
	for (Enumeration e = in.questions.elements() ; e.hasMoreElements() ;) {
		...
		if (info != null) {
			...
			out.addAnswer(in, new DNSRecord.Service(q.name, TYPE_SRV, CLASS_IN | CLASS_UNIQUE, 60, info.priority, info.weight, info.port, info.name));
		    }
		    if ((q.type == TYPE_TXT) || (q.type == TYPE_ANY)) {
			out.addAnswer(in, new DNSRecord.Text(q.name, TYPE_TXT, CLASS_IN | CLASS_UNIQUE, 60, info.text));
		    }
		    if ((q.type == TYPE_A) || (q.type == TYPE_ANY)) {
			out.addAnswer(in, new DNSRecord.Address(q.name, TYPE_A, CLASS_IN | CLASS_UNIQUE, 60, info.getIPAddress()));
		    }
		}
	    } 
	}
	...
	public void run()
	{
		...
		while (!done) {
			...
			if (msg.isQuery()) {
				...
				handleQuery(msg, packet.getAddress(), MDNS_PORT);
			    }
			} else {
			    handleResponse(msg);
			}
